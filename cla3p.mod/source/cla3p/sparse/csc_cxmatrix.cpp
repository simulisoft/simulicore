...
/*-------------------------------------------------*/
template <typename T_Int, typename T_Scalar>
alias::VirtualStrided_csc<T_Int, typename TypeTraits<T_Scalar>::real_type> 
CxMatrix<T_Int,T_Scalar>::real()
{
    T_RScalar *realValues = reinterpret_cast<T_RScalar*>(this->values());
    return alias::VirtualStrided_csc<T_Int,T_RScalar>(this->nrows(), 
                                                      this->ncols(),
                                                      this->colptr(), 
                                                      this->rowidx(),
                                                      realValues, 
...
/*-------------------------------------------------*/
template <typename T_Int, typename T_Scalar>
alias::GuardedStrided_csc<T_Int, typename TypeTraits<T_Scalar>::real_type> 
CxMatrix<T_Int,T_Scalar>::real() const
{
    const T_RScalar *realValues = reinterpret_cast<const T_RScalar*>(this->values());
    return alias::VirtualStrided_csc<T_Int,T_RScalar>::view(this->nrows(), 
                                                            this->ncols(), 
                                                            this->colptr(), 
                                                            this->rowidx(),
                                                            realValues, 
